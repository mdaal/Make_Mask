...
def Init_DB(Folder_Name,File_Name = ":memory:",Rebuild_DB = True):
	...
	if Rebuild_DB = False, this function  uses exisiting databases without overwriting them. Global variables pointing to these databases are set.
	...
	if Rebuild_DB == True:
		...
		connection.close()
		num_resonators = Get_Mask_Data("SELECT resonator_id FROM Resonators ORDER BY resonator_id DESC",fetch = 'one')[0]
		num_simulation = Get_Mask_Data("SELECT simid FROM Simulation_Geometries ORDER BY simid DESC",fetch = 'one')[0]
